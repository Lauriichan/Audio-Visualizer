...
export(Array, String) var itemList setget _set_item_list;
export(String) var selected setget _set_selected;
	
func _set_item_list(var value):
	itemList = value;
	_update_item_list();
...
func _ready():
	_update_item_list();
	var _ignore = get_popup().connect("id_pressed", self, "_item_pressed");
