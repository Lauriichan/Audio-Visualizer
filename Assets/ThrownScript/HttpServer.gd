...
func _respond(_request: Request) -> Response:
	var query = _request.request_query;
	...
	for part in queryParts:
		if part.find("=") == -1:
			continue;
		var data = part.split("=", false, 1);
		if data[0] == "code":
			f_code = data[1];
		if data[0] == "state":
			f_state = data[1];
	if f_code == null or f_state == null:
		_code = [];
	else:
		_code = [f_code, f_state];
	var response = Response.new();
	response.headers = ["Content-Type: text/html"]
	response.body = "<html><body>You can close this now</body></html>".to_utf8();
	return response;
